...
- Build now emits:
  ...
  - `.vercel/output/config.json` (routes: filesystem for assets + /\_\_server for all paths)
- npm install + npm run build verified exit 0 multiple times.
- .grok/, node*modules/, dist/, 00*\*/ zips, .vercel/ (build) never staged.
...
4. Confirmed .vercel/output/functions/\_\_server.func/index.mjs exists
...
See also ERRORS.md / FIXED*ERRORS.md / GROK*\* reports.
