    ...
    <Accordion type="single" collapsible className="k2k-faq-accordion-list w-full">
      {items.map((item, i) => (
        ...
        >
          <AccordionTrigger
            className={cn(
              "relative min-h-[3.25rem] px-5 py-4 text-left text-base font-medium text-ink transition-colors duration-300 hover:no-underline sm:min-h-[3.5rem] sm:px-6 sm:py-5 sm:text-lg",
          ...
          <AccordionContent className="px-5 pb-5 text-sm leading-relaxed text-k2k-navy/90 sm:px-6 sm:pb-6">
            <div className="border-t border-k2k-blue/10 pt-4 pb-1">{item.a}</div>
          </AccordionContent>
        </AccordionItem>
      ))}
    </Accordion>
